...
########################## test 1 ############################
"""
brief: 通过skimage.io读取指定目录的图片，并使用matplotlib.pyplot
       展现
       读取的image是numpy的数组格式，相关字段与函数演示
"""
def test1():
...
########################## test 3 ############################
"""
brief: 演示通过numpy数组的元素修改，来修改图片
       使用numpy构造一个数组，并展现出来
       为该图片增加一些噪声数据，并剪切图片，并消除噪声
       演示了保存图片的方法
"""
def test3():
...
########################## test 4 ############################
"""
brief: 通过skimage.data来读取图片，data_dir是本地目录
       演示2值灰度图的方法
       演示将图片划分为3个值，并打label的方法
       演示图片格式转换，色彩空间转换
"""
def test4():
...
########################## test 5 ############################
"""
brief: 通过data获取宇航员的图片
       演示了matplotlib.pyplot展现多个子图的能力
       演示了显示不同通道的图片的区别
TODO:  单通道的图，为什么要通过plt.cm.gray来展现？
"""
def test5():
...
########################## test 6 ############################
"""
brief: 演示从指定一个或多个目录下读取所有图片，并调用回调函数
       回调函数只有在对应数据被访问时，会调用
       演示了图像resize
TODO: resize与rescale的区别是什么?
      窗口的tab是怎么回事？因为上一个imshow没有plt.show出来么？
"""
def test6():
    def convert_gray(f):
        rgb = io.imread(f)
...
########################## test 7 ############################
"""
brief: 演示了resize图片，rescale图片，rotate图片
TODO: resize与rescale的区别是什么?
"""
def test7():
    img = data.camera()
...
########################## test 8 ############################
"""
brief: 演示图像金字塔的构造，并拼接到一块白图上
       注意图像的dtype是uint8还是float，必须一致，才能拼接上
"""
def test8():
    img = data.astronaut()
...
########################## test 9 ############################
"""
brief: 演示了图像亮度调整，以及强度的调整
       强度调整，看不出区别啊? 将强度拉伸到指定的range区间
"""
def test9():
...
########################## test 11 ############################
"""
brief: 演示了直方图均衡化，使得图片对比更明显，看的更清晰
       将灰度值比较密集的像素点的灰度值拉伸开
       将灰度值比较稀疏的像素点的灰度值压缩
       具体参考输出的灰度图进行理解
TODO: 为何均衡化后的图片，直方图返回的n的总数不等于1？
"""
def test11():
    img = data.moon()
    plt.figure(num="moon")
